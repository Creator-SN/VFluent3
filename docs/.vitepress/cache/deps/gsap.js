...
// node_modules/.pnpm/gsap@3.1.1/node_modules/gsap/gsap-core.js
...
var _nextGCFrame = 30;
var _harnessPlugins = [];
var _callbackNames = "onComplete,onUpdate,onStart,onRepeat,onReverseComplete,onInterrupt";
...
var _addToTimeline = function _addToTimeline2(timeline2, child, position) {
  ...
  if (child._time || !child._dur && child._initted) {
    var curTime = (timeline2.rawTime() - child._start) * child._ts;
    if (!child._dur || _clamp(0, child.totalDuration(), curTime) - child._tTime > _tinyNum) {
      child.render(curTime, true);
    }
  }
  _uncache(timeline2);
  if (timeline2._dp && timeline2._time >= timeline2._dur && timeline2._ts && timeline2._dur < timeline2.duration()) {
    var tl = timeline2;
    while (tl._dp) {
...
var _parsePosition = function _parsePosition2(animation, position, useBuildFrom) {
...
var wrapYoyo = function wrapYoyo2(min, max, value) {
  ...
  return _isArray(min) ? _wrapArray(min, wrapYoyo2(0, min.length - 1), max) : _conditionalReturn(value, function(value2) {
    value2 = (total + (value2 - min) % total) % total;
...
var mapRange = function mapRange2(inMin, inMax, outMin, outMax, value) {
  ...
  return _conditionalReturn(value, function(value2) {
    return outMin + (value2 - inMin) / inRange * outRange;
  });
};
...
var splitColor = function splitColor2(v, toHSL) {
...
var _formatColors = function _formatColors2(s, toHSL) {
...
var _ticker = function() {
  var _getTime = Date.now, _lagThreshold = 500, _adjustedLag = 33, _startTime = _getTime(), _lastUpdate = _startTime, _gap = 1 / 60, _nextTime = _gap, _listeners = [], _id, _req, _raf, _self, _tick = function _tick2(v) {
...
var Animation = function() {
  ...
  _proto.duration = function duration(value) {
    var isSetter = arguments.length, repeat = this._repeat, repeatCycles = repeat > 0 ? repeat * ((isSetter ? value : this._dur) + this._rDelay) : 0;
  ...
  _proto.totalDuration = function totalDuration(value) {
    ...
    var repeat = this._repeat, isInfinite = (value || this._rDelay) && repeat < 0;
  ...
  _proto.totalTime = function totalTime(_totalTime, suppressEvents) {
    ...
    var parent = this.parent || this._dp, start;
...
var Timeline = function(_Animation) {
    ...
    if (tTime !== this._tTime || force || crossingStart) {
      ...
      if (this._repeat) {
        ...
        if (iteration !== prevIteration && !this._lock) {
          ...
          this.vars.repeatRefresh && !isYoyo && this.getChildren().forEach(function(child2) {
            return child2.invalidate();
          });
  ...
  _proto2.tweenTo = function tweenTo(position, vars) {
    var tl = this, endTime = _parsePosition(tl, position), startAt = vars && vars.startAt, tween = Tween.to(tl, _setDefaults({
      ease: "none",
      lazy: false,
      time: endTime,
  ...
  _proto2.totalDuration = function totalDuration(value) {
    var max = 0, self = this, child = self._last, prevStart = _bigNum, repeat = self._repeat, repeatCycles = repeat * self._rDelay || 0, isInfinite = repeat < 0, prev, end;
    if (!arguments.length) {
      if (self._dirty) {
        while (child) {
          prev = child._prev;
...
var _staggerPropsToSkip = (_staggerTweenProps + ",id,stagger,delay,duration,paused").split(",");
var Tween = function(_Animation2) {
  _inheritsLoose(Tween2, _Animation2);
  function Tween2(targets, vars, time) {
    var _this3;
    ...
    var _this3$vars = _this3.vars, duration = _this3$vars.duration, delay = _this3$vars.delay, immediateRender = _this3$vars.immediateRender, stagger = _this3$vars.stagger, overwrite = _this3$vars.overwrite, keyframes = _this3$vars.keyframes, defaults2 = _this3$vars.defaults, parsedTargets = _isArray(targets) && _isNumber(targets[0]) ? [targets] : toArray(targets), tl, i, copy, l, p, curTarget, staggerFunc, staggerVarsToMerge;
...
var _gsap = {
  ...
  registerEffect: function registerEffect(_ref) {
    ...
    _effects[name] = function(targets, vars) {
      return effect(toArray(targets), _setDefaults(vars || {}, defaults2));
    };
    if (extendTimeline) {
...
// node_modules/.pnpm/gsap@3.1.1/node_modules/gsap/CSSPlugin.js
var _win2;
var _doc2;
var _docElement;
var _pluginInitted;
...
var _renderCSSProp = function _renderCSSProp2(ratio, data) {
  return data.set(data.t, data.p, ~~((data.s + data.c * ratio) * 1e3) / 1e3 + data.u, data);
};
var _renderPropWithEnd = function _renderPropWithEnd2(ratio, data) {
  return data.set(data.t, data.p, ratio === 1 ? data.e : ~~((data.s + data.c * ratio) * 1e3) / 1e3 + data.u, data);
};
var _renderCSSPropWithBeginning = function _renderCSSPropWithBeginning2(ratio, data) {
  return data.set(data.t, data.p, ratio ? ~~((data.s + data.c * ratio) * 1e3) / 1e3 + data.u : data.b, data);
};
...
var _checkPropPrefix = function _checkPropPrefix2(property, element) {
...
var _convertToUnit = function _convertToUnit2(target, property, value, unit) {
  var curValue = parseFloat(value) || 0, curUnit = (value + "").trim().substr((curValue + "").length) || "px", style = _tempDiv.style, horizontal = _horizontalExp.test(property), isRootSVG = target.tagName.toLowerCase() === "svg", measureProperty = (isRootSVG ? "client" : "offset") + (horizontal ? "Width" : "Height"), amount = 100, toPixels = unit === "px", px, parent, cache, isSVG;
...
var _renderClearProps = function _renderClearProps2(ratio, data) {
  if (data.tween && data.tween._time === data.tween._dur) {
    var target = data.t, style = target.style, props = data.u, prop, clearTransforms, i;
...
var _getMatrix = function _getMatrix2(target, force2D) {
  var cache = target._gsap, style = target.style, matrix = _getComputedTransformMatrixAsArray(target), parent, nextSibling, temp, addedToDOM;
...
// node_modules/.pnpm/gsap@3.1.1/node_modules/gsap/index.js
