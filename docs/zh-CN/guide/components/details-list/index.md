...
| 属性                | 类型    | 必填  | 默认值                                      | 说明                                                                                          |
|:-------------------:|:-------:|:-----:|:-------------------------------------------:|:---------------------------------------------------------------------------------------------:|
| modelValue          | array   | 否    | []                                          | 列表数据，数据格式详见数据格式表。                                                            |
| head                | array   | 否    | []                                          | 表头数据，数据格式详见数据格式表。                                                            |
| group               | array   | 否    | []                                          | 分组数据，数据格式详见数据格式表。                                                            |
| filter              | object  | 否    | () => { return { key: 'any', value: '' }; } | 搜索筛选，数据格式详见数据格式表，控制表显示哪些搜索字段。                                    |
| showGroup           | boolean | 否    | false                                       | 是否开启按组分割显示，开启后排序功能将失效，默认不开启，若 group 为空，则开启后表格内容为空。 |
| autoHeight          | boolean | 否    | false                                       | 是否开启自动高度。                                                                            |
| foreground          | string  | 否    | ''                                          | 表格前景色。                                                                                  |
| headBackground      | string  | 否    | ''                                          | 表头背景，不会修改点燃颜色，若要修改点燃颜色请自定义 fv-custom-head 类样式。                  |
| compact             | boolean | 否    | false                                       | 是否开启收缩模式。                                                                            |
| multiSelection      | boolean | 否    | false                                       | 是否开启多选模式。                                                                            |
| allowDrag           | boolean | 否    | false                                       | 是否开启拖动排序。                                                                            |
| rowCSS              | String  | No    | N/A                                         | 客制化每一行的样式。                                                                          |
| rightMenuWidth      | number  | 否    | 200                                         | 右键菜单宽度设置。                                                                            |
| showRightMenu       | boolean | 否    | true                                        | 是否启用右键菜单。                                                                            |
| rightMenuBackground | string  | 否    | ''                                          | 右键菜单背景色。                                                                              |
| rightMenuFullExpand | boolean | 否    | false                                       | 是否启用全展开右键菜单显示动画。                                                              |
| theme               | string  | 否    | 'global'                                    | 主题样式。支持 `global`、`light`、`dark`、`system` 和 `custom`。                              |
| rowCss              | string  | 否    | ''                                          | 参见组件的 `rowCss` 选项。                                                                    |
