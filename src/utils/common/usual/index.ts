export class SDate {
    ...
    static Parse(DateString: string) {
        ...
        if (timeArray.length <= 3) return this.StringToDate(DateString);
    ...
    static StringToDateTime(DateStr: string) {
        let reg = /\d+/g;
        let timeArray = new Array();
    ...
    static DisTime(startDate: Date, endDate: Date) {
        return endDate.getTime() - startDate.getTime();
    }
    static DisDay(startDate: Date, endDate: Date) {
        let dis = SDate.DisHour(startDate, endDate);
        if (Math.abs(dis) > 24) return Math.floor(dis / 24);
        else return startDate.getDate() == endDate.getDate() ? 0 : 1;
    }
    static DisHour(startDate: Date, endDate: Date) {
        let t = SDate.DisTime(startDate, endDate) / (3600 * 1000);
        return Math.floor(t);
    }
    static DisMinute(startDate: Date, endDate: Date) {
        let t = SDate.DisTime(startDate, endDate) / (60 * 1000);
        return Math.floor(t);
    }
    static DisSecond(startDate: Date, endDate: Date) {
        return Math.round(SDate.DisTime(startDate, endDate) / 1000);
    }
    static Compare(current: Date, compare: Date) {
        if (current.getTime() - compare.getTime() > 0) return 1;
        else if (current.getTime() == compare.getTime()) return 0;
        return -1;
    }
    static IsSameDate(current: Date, compare: Date) {
        if (
...
export class SUtility {
    static ScrollToLoadInit(e: HTMLElement, callback: any = null, offset = 0) {
        try {
    ...
    // static xhr(
    //     params = {} //{ url: '', data: [{ name: '', value: '' }], success: function (responseText) {}, error: function (responseText) {}, progress: function (loaded, currentLoaded) {} } //
    // ) {
