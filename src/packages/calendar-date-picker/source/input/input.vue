<template>
    ...
    >
        ...
        ></fv-reveal-container>
        <div
            class="calendar-picker-drop-down-container"
            :style="{borderRadius: `${borderRadius}px`}"
        >
            <slot
                name="input"
            ...
            >
                <input
                    ...
                    readonly
                    :value="inputValue"
                    :style="{ color: inputForeground, background: inputBackground, borderRadius: `${borderRadius}px`}"
                />
                <i
    ...
    computed: {
        ...
        inputValue() {
            ...
            for (let i = 0; i < this.choosenValue.length; i++) {
                if (i > 0) result += `,`;
                result += `${this.choosenValue[i].year}-${
                    this.choosenValue[i].month + 1
                }-${this.choosenValue[i].no}`;
            }
            return result;
        },
        borderLightColor() {
