...
export default {
    ...
    methods: {
        ...
        globalMoveEventListener(e) {
            if (this.revealContainer === false) {
                for (let item of this.revealCache.revealHandlerList) {
        ...
        moveEvent(e) {
            ...
            let distance = this.distanceOfElement(
                this.$el,
                e.clientX,
                e.clientY
            );
            if (
                distance.left > this.debounceDistance ||
                distance.top > this.debounceDistance ||
                distance.right > this.debounceDistance ||
                distance.bottom > this.debounceDistance
            )
                return;
            this.disEl.x = x;
            this.disEl.y = y;

            this.inside = this.moveInsideElement(
                this.$el,
                e.clientX,
                e.clientY
            );
    ...
    beforeDestroy() {
        if (this.revealContainer === false) {
            ...
            let index = this.revealContainer.revealHandlerList.findIndex(
                (item) => item.id == this.id
            );
            if (index > -1)
                this.revealContainer.revealHandlerList.splice(index, 1);
        }
