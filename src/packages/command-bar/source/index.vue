...
<template>
    <div ref="root" class="fv-CommandBar" :class="[theme]" :style="{ background: background }">
        <div class="left-command-bar-container">
            <span v-show="callFunction(item.show)" v-for="(item, index) in thisOptions" class="command-bar-item"
                ...
                :key="index" :title="callFunction(item.name)" @click="itemClick($event, item)">
                <span v-show="callFunction(item.type) !== 'more'" class="s1-container">
                    <fv-reveal-container v-if="callFunction(item.type) != 'divider' && !callFunction(item.disabled)"
                        :parent="() => $el" class="fv-command-bar-reveal-container"
                        :backgroundColor="backgroundLightColor" :borderColor="borderLightColor" :borderGradientSize="80"
                        :borderWidth="1" :borderRadius="6"></fv-reveal-container>
                    <i class="ms-Icon icon" :class="[`ms-Icon--${callFunction(item.icon)}`]"
                        :style="{ color: callFunction(item.iconColor) }"></i>
                    <p v-show="!compact" class="name">
                        {{ callFunction(item.name) }}
                    </p>
                    <i v-show="item.secondary !== undefined && item.secondary.length > 0"
                        class="ms-Icon ms-Icon--ChevronDown icon"></i>
                </span>
                <span v-show="callFunction(item.type) === 'more'" class="s1-container">
